...
- [x] 2.4 Реализовать rules-only degraded mode только при явном разрешении consumer policy, иначе `503`; observable check: degradation unit tests проходят.
...
- [x] 5.7 Реализовать regex candidates для dates, включая текстовые даты; observable check: table tests с synthetic fixtures проходят.
- [x] 5.8 Реализовать regex candidates для address и postal code; observable check: table tests с synthetic fixtures проходят.
- [x] 5.9 Реализовать regex candidates и context validators для CVV/PIN/cardholder; observable check: positive/negative unit tests проходят.
...
- [x] 6.3 Реализовать ownership scoring с positive/negative context, shared `owner_id`, organization context и `review_recommended`; observable check: hard negatives tests проходят.
...
- [x] 8.2 Реализовать восстановление глобальных UTF-8 offsets из окон; observable check: offset reconstruction unit tests проходят.
- [x] 8.3 Реализовать подсчёт токенов через tokenizer выбранной модели и acceptance для входа до 100 000 токенов; observable check: token-count acceptance test проходит.
...
- [x] 9.4 Реализовать воспроизводимый Gitleaks gate: локальная команда и CI, сканирование Git history и текущих файлов, redacted output, без baseline для чистого нового репозитория; observable check: оба режима завершаются без findings.
- [x] 9.5 Реализовать минимальный Semgrep CE SAST gate: без cloud token, explicit ruleset (не `--config auto`), metrics off, blocking только ERROR; локальная команда и CI; observable check: scan текущего Go/Python source проходит.
...
- [x] 11.1 Реализовать бенчмарк целевой нагрузки 1000 RPS при полном прогоне около 5 минут; собирать p50/p95/p99 и проверять product target latency не более 1 секунды, поясняя, что Appendix называет его целевым ориентиром; observable check: benchmark проходит и отчёт содержит p50/p95/p99.
...
- [x] 12.4 Добавить E2E demo mask -> external processing preserving tokens -> restore; observable check: automated E2E command проходит.
- [x] 12.5 Добавить Dockerfile и docker-compose для Go service и Python worker; observable check: `docker compose config` проходит и startup документирован.
- [x] 12.6 Добавить README с запуском Go-сервиса, Python worker-а, fast mode, конфигом vault, demo round trip и инструкцией настройки consumer policy не более пяти предложений; observable check: команды из README проверены вручную или smoke script.
- [x] 12.7 Реализовать минимальный Go runtime coordinator с инъецируемым/настраиваемым LLM client boundary и покрыть его automated E2E полного пути mask -> LLM -> demask: fake LLM подтверждает, что наружу к LLM уходит только защищённый текст, stage errors (tokenization/vault/LLM/detokenization) fail closed без plaintext fallback, а `POST /process` остаётся отдельным mask/restore контуром и сам LLM не вызывает; observable check: automated E2E команда проходит.
- [x] 12.8 Реализовать runnable downstream-LLM wiring: stdlib-only HTTP client для chat-completions-совместимого endpoint (`PII_LLM_URL`, `PII_LLM_MODEL`, опц. `PII_LLM_API_KEY`, `PII_LLM_TIMEOUT`), исходящий JSON с моделью, короткой system-инструкцией о byte-for-byte сохранении непрозрачных токенов и защищённым текстом как user message, парсинг `choices[0].message.content`, `Authorization: Bearer` только при непустом ключе, фиксированные безопасные ошибки без утечки текста/токенов/ключа/тела ответа/деталей upstream, context/timeout, HTTP(S), reject non-2xx/malformed/trailing JSON/empty content/oversized response; LLM-группа опциональна целиком (при отсутствии URL и model старт успешен, `/v1/runtime/chat` fail-closed 503, `/process` работает отдельно; частичная конфигурация — ошибка валидации при старте); wiring в `Pipeline.RuntimeCoordinator` и регистрация `WithRuntime` в `cmd/pii-service`; production-wiring HTTP тест с httptest downstream (получает токены, но не синтетический plaintext; финальный runtime-ответ восстанавливает значения; upstream failure fail closed без утечки маркеров; `/process` не вызывает downstream LLM); обновление docker-compose/README/runbook с четырьмя переменными и исполняемым synthetic smoke для `/v1/runtime/chat`; observable check: `go test ./...` проходит и smoke-команда выполнима.
